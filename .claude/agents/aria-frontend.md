...
- `useTTS.ts` — `speak(text, emotion?)` POSTs to `API_BASE/api/tts` (from `lib/config.ts`), forwarding ARIA's `avatar_emotion` as `emotion` when present, plays the returned MP3 via `new Audio`, falls back to `speechSynthesis` (`speakWithBrowser`). Exports module-level `ttsAudioRef` and `speakRef`. Sends `tts_mute`/`tts_unmute` over WS around playback.
