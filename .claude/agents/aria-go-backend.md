...
- `backend/internal/cognition/client.go` — `Client.Complete()` enriches the request with the owner's working memory (last 5 symbolic inferences) and POSTs to Python `PythonBaseURL/api/cognition` (30s timeout), sending `X-Aria-Owner` + `X-Internal-Auth` + request id. Pushes returned symbolic inference into owner-scoped working memory; derives `AvatarEmotion` via keyword scan. It holds NO episodic/profile memory: Python retrieves that from ChromaDB on every turn (S3), and the `episodic_memory` list in the response is passed through to the browser for display only.
...
**Cognition request:** Browser `POST /api/cognition` (Bearer token) → auth middleware sets the owner → rate limiter → `Handler.ServeHTTP` caps the body, validates `message`+`session_id`, calls `Client.Complete` → enriches with the owner's working memory → POSTs to Python `PythonBaseURL/api/cognition` with `X-Aria-Owner` + `X-Internal-Auth` + request id → stores symbolic inference → returns `CognitionResponse` (incl. passthrough `spatial_event` `json.RawMessage`).
...
- **No Go-side semantic memory.** The former per-owner episodic replay cache in `cognition/client.go` was removed (S3): it re-sent the previous turn's facts and resurrected deleted memory. Only the working-memory ring (symbolic inferences, in-process, per owner) lives in Go; `DELETE /api/memory` clears it for the owner after Python confirms the delete. Memory controls proxied at the edge: `GET /api/memory/export`, `DELETE /api/memory`, `DELETE /api/memory/{entry_id}` (rate-limited, owner from the verified token, query string never forwarded).
