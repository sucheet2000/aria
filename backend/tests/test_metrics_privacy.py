...
def _reset() -> None:
    ...
    # branches, and a fixture that assumes one of them fails for a reason that
    # has nothing to do with the property under test.
    m = MetricsCollector()
    for attr in ("_gesture_events", "_cognition_timeout"):
    ...
    def test_4_every_published_key_is_server_chosen(self) -> None:
        ...
        for field, permitted in allowed.items():
            if field not in snap:
                continue  # not every branch emits every metric
        ...
        # where this build has them.
        for field in ("token_cost", "prompt_cache", "llm_response"):
            for key in snap.get(field, {}):
