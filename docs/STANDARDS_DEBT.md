...
| `API-2` | `POST /api/cognition` carries **no version** (no path version, no version field), so any wire rename is a breaking change that fails silently: R2 renamed `vision_state.confidence` → `vision_state.emotion_confidence` across TS/Go/pydantic in one change, and a stale frontend bundle would send the old key, Go would drop it, pydantic would default to `None`, and conflict detection would go dark. **Deploy frontend and backend atomically** for such changes until the route is versioned. |
