...
|------|------|----------|-------|--------|
| `SCALE-1` | Perception reads the **container's physical camera/mic** — the headline feature cannot run in the cloud; capture must move browser-side | critical | `backend/app/pipeline` vision/audio workers | **Phase A.2 — in progress** (A.2a browser vision building; A.2b audio next) |
| `ARCH-1/2/3` | Business logic (emotion) in the transport layer; duplicated `broadcastFrame` + 3 near-identical proxy handlers + `localhost:8000` in 3 files; 4 divergent emotion enums | low–med | Go + frontend | after A.2 (much of the dup vanishes when the dead transports are deleted) |
