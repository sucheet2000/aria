...
describe("useWorldModel", () => {
  ...
  it("addAnchor inserts an anchor by anchor_id", () => {
    useWorldModel.getState().addAnchor({ anchor_id: "a1", label: "person", x: 1, y: 2, z: 3 });
    const { anchors } = useWorldModel.getState();
    expect(anchors.size).toBe(1);
    expect(anchors.get("a1")).toEqual({ anchor_id: "a1", label: "person", x: 1, y: 2, z: 3 });
  });

  it("addAnchor overwrites an existing anchor with the same anchor_id", () => {
    useWorldModel.getState().addAnchor({ anchor_id: "a1", label: "person", x: 1, y: 0, z: 0 });
    useWorldModel.getState().addAnchor({ anchor_id: "a1", label: "object", x: 4, y: 5, z: 6 });
    const { anchors } = useWorldModel.getState();
    expect(anchors.size).toBe(1);
    expect(anchors.get("a1")?.label).toBe("object");
  });

  it("removeAnchor deletes by anchor_id", () => {
    useWorldModel.getState().addAnchor({ anchor_id: "a1", label: "place", x: 0, y: 0, z: 0 });
    useWorldModel.getState().addAnchor({ anchor_id: "a2", label: "event", x: 1, y: 1, z: 1 });
  ...
  it("updateAnchor changes label while preserving position", () => {
    useWorldModel.getState().addAnchor({ anchor_id: "a1", label: "person", x: 2, y: 3, z: 4 });
  ...
  it("setAnchorVelocity sets velocity on the anchor", () => {
    useWorldModel.getState().addAnchor({ anchor_id: "a1", label: "person", x: 0, y: 0, z: 0 });
  ...
  it("multiple anchors coexist independently", () => {
    const anchors = [
      { anchor_id: "a1", label: "person", x: 0, y: 0, z: 0 },
      { anchor_id: "a2", label: "object", x: 1, y: 1, z: 1 },
      { anchor_id: "a3", label: "place", x: 2, y: 2, z: 2 },
    ];
